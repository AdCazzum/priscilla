...
echo "🟢 Injecting model config" | tee -a $IEXEC_OUT/stdout.txt
sed -i "s/OLLAMA_MODEL_NAME_TO_REPLACE/$MODEL_NAME/" .env

# Initialize event input file if it doesn't exist
if [ ! -f "/iexec_in/events.json" ]; then
    echo "📝 Creating empty events file" | tee -a $IEXEC_OUT/stdout.txt
    echo '[]' > /iexec_in/events.json
fi

echo "🟢 Start Eliza Agent (Event-Driven Mode)" | tee -a $IEXEC_OUT/stdout.txt
pnpm start --character="characters/character.json" > >(tee -a "$IEXEC_OUT/stdout.txt") 2> >(tee -a "$IEXEC_OUT/stderr.txt" >&2) &
ELIZA_PID=$!

# ⏳ Wait for 5 minutes
echo "⏳ The agent will run for 5 minutes to process events and generate responses..." | tee -a $IEXEC_OUT/stdout.txt
sleep 300
